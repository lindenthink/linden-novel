...
use crate::db::repo::{embedding_chunk_repo, embedding_repo, foreshadow_repo, inspiration_repo, project_repo};
...
pub async fn delete(pool: &SqlitePool, id: &str) -> Result<(), AppError> {
    ...
    // 清理项目级灵感
    if let Err(e) = inspiration_repo::delete_by_project(pool, id).await {
        tracing::warn!("Failed to clean inspirations for project {}: {}", id, e);
    }
